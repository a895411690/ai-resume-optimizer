...
type EntitlementSource = "authenticated" | "demo_daily" | "free_once" | "vip";
...
export async function getEntitlementSummary(userId: string): Promise<EntitlementSummary> {
  ...
  const { data, error } = await getSupabaseAdmin()
    .from("user_entitlements")
    .select("vip_expires_at,free_optimization_used")
    .eq("user_id", userId)
    .single();
  ...
  const isVip = Boolean(vipExpiresAt && new Date(vipExpiresAt).getTime() > Date.now());
...
export async function reserveOptimizationAccess(userId: string, action: Exclude<AiAction, "diagnose">): Promise<AccessReservation> {
  ...
  const entitlementSource: EntitlementSource = entitlement.isVip ? "vip" : "free_once";

  if (!entitlement.isVip && entitlement.freeOptimizationUsed) {
...
export async function markAiUsageFailed(eventId: string | null) {
  ...
  if (data?.entitlement_source === "free_once" && data.user_id) {
    await admin
      .from("user_entitlements")
